...
export default function transform(raw: any, t: (input: Text) => string) {
  ...
  if (raw?.sort?.groupList && raw?.sort?.categoryList) {
    ...
    componentList = raw.components?.filter(component => {
      ...
      // npm 信息
      const npm = component?.npm || null;
      const version = component?.npm?.version || '0.0.0';
      component.snippets.forEach(snippet => {
        const snippetId = `${t(component.group)}_${t(component.category)}_${component.componentName}_${snippet.title}`;
        snippet.id = npm ? `${npm.package}_${npm.exportName}_${npm.subName}_${version}_${snippet.title}` : snippetId;
        snippet.schema.version = version;
