...
async function collectMetaInfo() {
  try {
    // 读取comp文件夹下的所有子文件夹
    const subdirs = await fs.readdir(compFolder, { withFileTypes: true });
    ...
    // 输出到meta.json文件
    const metaJsonPath = path.join(targetFolder, "meta.json");
    await fs.writeFile(
      metaJsonPath,
      JSON.stringify(metaInfoArray, null, 2),
      "utf8"
    );
    console.log("meta.json file has been created successfully.");
  } catch (err) {
